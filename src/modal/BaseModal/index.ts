...
export type SchemaDefinition = Record<string, FieldSchema | 'string' | 'number' | 'object'>;
...
export abstract class BaseModel  {
    ...
    private validateSchema(body: any, schema: SchemaDefinition, errors: string[], path: string = '') {
        Object.keys(schema).forEach(key => {
            ...
            if (fieldSchema.type === 'array') {
                if (!Array.isArray(value)) {
                    ...
                    if (fieldSchema.minItems && value.length < fieldSchema.minItems) {
                        errors.push(`${currentPath} must contain at least ${fieldSchema.minItems} items.`);
                    }
                    if (fieldSchema.maxItems && value.length > fieldSchema.maxItems) {
                        errors.push(`${currentPath} can contain at most ${fieldSchema.maxItems} items.`);
                    }
                    if (fieldSchema.items) {
                        value.forEach((item: any, index: number) => {
                            this.validateSchema(item, { item: fieldSchema }, errors, `${currentPath}[${index}]`);
                        });
                    }
                }
            }
            ...
            if (fieldSchema.type === 'object') {
                if (typeof value !== 'object' || Array.isArray(value)) {
                    ...
                    this.validateSchema(value, fieldSchema.properties, errors, currentPath);
                }
            }
            
            if (fieldSchema.minLength && typeof value === 'string' && value.length < fieldSchema.minLength) {
                errors.push(`${currentPath} must be at least ${fieldSchema.minLength} characters long.`);
            }
            
            if (fieldSchema.maxLength && typeof value === 'string' && value.length > fieldSchema.maxLength) {
                errors.push(`${currentPath} must be at most ${fieldSchema.maxLength} characters long.`);
            }
        });
