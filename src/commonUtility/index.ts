...
class CommonUtility {
   ...
   validateRequestBody(body: any, schema: Record<string, string>): { valid: boolean, errors: string[] } {
        const errors: string[] = [];
    ...
    genrateJWTToken(payload:any,options?:SignOptions){
        return jsonwebtoken.sign(payload, singingKey, {
          ...
          allowInvalidAsymmetricKeyTypes:options?.allowInvalidAsymmetricKeyTypes?? false,
          issuer:options?.issuer ?? "dibendugorai",
        });
    }

    verifyJWTToken(token:string, callback?:(err:any,info:any)=>void,options?:VerifyOptions,) {
        return jsonwebtoken.verify(token, singingKey, {
